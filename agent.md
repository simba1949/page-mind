...
- 解决 bug 前，必须以最新 `master` 分支为基线创建独立修复分支，不直接在功能或版本分支上修复。
- 分支名统一使用 `fix-YYYYMMDD-HHmm` 格式：`fix-` 前缀后拼接年月日时分，例如 `fix-20260909-1619`。
- 修复分支创建后，即使远程尚不存在对应的 `fix-*` 分支，也必须使用 `git push --set-upstream origin <fix-分支名>` 创建并推送到远程同名 `fix-*` 分支，并让本地分支跟踪该远程修复分支；禁止将修复分支直接推送或设置为跟踪远程 `master`。
- 创建前必须检查工作区和分支状态，禁止覆盖或丢弃已有未提交改动。
...
每次完成代码改动后，必须进行完整验收，不得只报告“测试通过”或“构建成功”：

- **资深架构师视角**：验证并评估系统架构、代码结构、依赖关系和可维护性。
- **资深 UI 设计工程师视角**：验收页面布局、视觉一致性、交互体验和异常状态。
- **资深测试工程师视角**：验证功能、异常场景、边界条件和回归影响；涉及性能的改动必须进行压测或等效性能验证。
- **资深安全工程师视角**：评估权限、数据安全、输入校验、敏感信息及常见安全漏洞。

输出时，对每个维度直接列出以下两项，并提供涉及代码的修改前后对比：

1. `【潜在风险】`
2. `【修改建议（附代码对比）】`

最后给出 `【综合评审结论】`，结论只能是：`通过`、`需修改` 或 `阻断`。若某项未实际验证，必须明确标注“未验证”，不得将其表述为已通过。
