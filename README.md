...
- **快捷操作** — 一键总结、解释、翻译当前页面，也可以配置自己的快捷 Prompt。
...
- **API 密钥本地保护** — 选择保存时，API 密钥使用 AES-GCM 保护后存储在当前 Chrome 用户配置中；该保护不等同于独立 OS 密钥链。默认情况下密钥仅存储在 `chrome.storage.session` 中，关闭浏览器即清除。
- **会话级聊天记录** — 聊天消息、网页上下文和附件仅存储在 `chrome.storage.session` 中，关闭浏览器即清除；新建对话会清理当前会话记录。
...
0.1.3 支持自定义快捷操作。项目版本号为 `0.1.3`，设置存储协议版本号为 `1.0`，存储 Schema 版本号为 `1.0`：只有已存在且不同的设置协议才会清理设置；协议标记缺失时会保留配置并补写当前协议，项目版本升级本身不会清理 API 配置。支持任意实现 `openai-chat`、`openai-responses` 或 `anthropic-messages` 协议的服务，端点可自行填写。
...
点击输入框上方的快捷按钮。打开设置中的“快捷操作”可以新增、编辑、删除和排序自定义操作，也可以选择保留哪些内置操作。自定义操作最多 10 个，名称和 Prompt 按原文保存，不随界面语言切换翻译。
...
自定义操作需要填写按钮名称和 Prompt。自定义项少于 3 个时，可以选择要保留的内置操作；自定义项达到 3 个后，快捷区以自定义项为主。
